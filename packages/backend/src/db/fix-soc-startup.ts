...
export async function fixSocPowerConsumption() {
  try {
    ...
    for (const soc of socs) {
      ...
      if (specs.powerConsumption) continue;

      await db.update(components)
        .set({ specifications: { ...specs, ...fix }, updatedAt: new Date() })
        .where(eq(components.id, soc.id));
      updated++;
    }

    if (updated > 0) {
      console.log(`[startup] Fixed powerConsumption for ${updated} SoC components`);
    }
  } catch (err) {
    console.error('[startup] SoC fix failed (non-fatal):', err);
  }
}
