...
export function ExecutiveInsightCard({
  ...
  if (isLoading) {
    return (
      <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20 rounded-xl p-6 h-full animate-pulse">
        <div className="h-5 bg-ink-100 rounded w-1/3 mb-4" />
  ...
  return (
    ...
    >
      ...
      {showTooltip && details && (
        <div className="absolute z-50 left-0 right-0 top-full mt-2 bg-white border border-ink-200 rounded-lg p-4 shadow-xl">
          <p className="text-sm text-ink-900 leading-relaxed">{details}</p>
        </div>
      )}
    </div>
  );
