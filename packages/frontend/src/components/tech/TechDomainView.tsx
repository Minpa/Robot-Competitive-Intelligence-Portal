...
interface DomainConfig {
  ...
  /** 영상 제외 필터 (키노트·인터뷰 등) */
  excludeVideoRegex?: RegExp;
  /** 영상을 별도 서브 페이지(/videos)로 분리 */
  splitVideos?: boolean;
...
const DOMAIN_CONFIGS: Record<string, DomainConfig> = {
  ...
  production: {
    ...
    newsFirst: true,
    splitVideos: true,
    excludeVideoRegex: /keynote|earnings|shareholder|annual meeting|interview|podcast|webinar|웨비나|기조연설/i,
...
export function TechDomainView({ view = 'main' }: { view?: 'main' | 'videos' }) {
  const params = useParams();
  const domain = params.domain as string;
  const config = DOMAIN_CONFIGS[domain];
  const isVideosView = view === 'videos';
    ...
    <AuthGuard>
      <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-6">
        <PageHeader
          ...
          titleKo={isVideosView ? `${config.titleKo} — 영상` : config.titleKo}
        ...
        {!isVideosView && config.alertsType && Array.isArray(alertsQuery.data) && alertsQuery.data.length > 0 && (
          <Panel
        ...
        {/* Videos */}
        {(isVideosView || !config.splitVideos) && (
        <Panel
          className={config.newsFirst && !isVideosView ? 'order-2' : undefined}
          kicker="Demo Videos"
