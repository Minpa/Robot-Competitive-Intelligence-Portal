...
export function ApplicationCaseCard({ caseData, onClick }: ApplicationCaseCardProps) {
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      case 'ended':
        return { label: '종료', color: 'bg-slate-700 text-slate-400 border-slate-600' };
      default:
        return { label: status, color: 'bg-slate-700 text-slate-400 border-slate-600' };
    }
  };
  ...
  return (
    <div
      className="bg-slate-800 rounded-lg border border-slate-700 p-4 hover:shadow-md hover:border-blue-300 transition-all cursor-pointer"
      onClick={onClick}
    >
      {/* 헤더: 제목 + 상태 배지 */}
      ...
      <div className="flex flex-wrap gap-2 mb-3 text-xs">
        {/* 환경 */}
        <span className="inline-flex items-center gap-1 px-2 py-1 bg-slate-700 rounded text-slate-300" title="환경">
      ...
      {caseData.robotImages && caseData.robotImages.length > 0 && (
        <div className="flex gap-2 mt-3 pt-3 border-t border-slate-700">
          {caseData.robotImages.slice(0, 3).map((img, i) => (
            <div
              key={i}
              className="w-12 h-12 bg-slate-700 rounded overflow-hidden group relative"
            >
              <img
                src={img}
                alt=""
