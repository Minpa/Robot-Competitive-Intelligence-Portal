...
export function RecentDeploymentEventsCard({
  ...
  return (
    <div className="bg-argos-surface rounded-lg shadow p-6">
      <h2 className="text-lg font-semibold text-argos-ink mb-4">최근 적용·도입 이벤트</h2>

      {/* 요약 숫자 3개 */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-blue-500/10 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-blue-400">{stats.newDeployments30d}</div>
          <div className="text-xs text-argos-muted">30일 신규 적용</div>
        </div>
        <div className="bg-green-500/10 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-green-400">{stats.pocToProductionCount}</div>
          <div className="text-xs text-argos-muted">PoC→상용 전환</div>
        </div>
        <div className="bg-red-500/10 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-red-400">{stats.failedCount}</div>
          <div className="text-xs text-argos-muted">실패/중단</div>
        </div>
      </div>

      {/* 임팩트 큰 사례 하이라이트 */}
      <div className="space-y-3">
        <h3 className="text-sm font-medium text-argos-inkSoft">주요 사례</h3>
        {highlights.length > 0 ? (
          highlights.slice(0, 2).map((highlight) => (
            <div
              key={highlight.id}
              className="flex items-start gap-3 p-3 bg-argos-bgAlt rounded-lg cursor-pointer hover:bg-argos-bgAlt transition-colors"
              onClick={() => onCaseClick?.(highlight.id)}
            >
              {/* 로고/아이콘 */}
              <div className="w-10 h-10 bg-argos-bgAlt rounded-lg flex items-center justify-center border border-argos-border flex-shrink-0">
                {highlight.logoUrl ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-medium text-sm text-argos-ink truncate">
                ...
                <p className="text-xs text-argos-muted line-clamp-2">{highlight.description}</p>
              </div>
            </div>
          ))
        ) : (
          <p className="text-sm text-argos-muted text-center py-4">최근 주요 사례가 없습니다.</p>
        )}
      </div>
    </div>
  );
