...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    default: return { dotColor: 'bg-ink-500', label: '미검증', cls: 'bg-ink-100 text-ink-500' };
  }
}
...
export function CiFreshnessPanel() {
  ...
  if (!data || data.length === 0) return <div className="text-ink-500 text-sm p-4">데이터 없음</div>;
  ...
  return (
    <div className="bg-white rounded-xl border border-ink-200 p-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-ink-900">데이터 신선도</h3>
        <div className="flex gap-3 text-xs">
          ...
          <span className="text-ink-500">전체 {total}</span>
        </div>
      </div>

      {/* Freshness bar */}
      <div className="w-full h-2 bg-ink-100 rounded-full mb-4 overflow-hidden flex">
      ...
      <div className="space-y-3">
        {Object.entries(byLayer).map(([layerName, { icon, items }]) => {
          ...
          return (
            <div key={layerName} className="bg-ink-100 rounded-lg p-3">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-1 min-w-0">
                        ...
                        <span className="text-[11px] text-ink-700 truncate">{item.competitorName}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className="text-[10px] text-ink-500">{tierLabels[item.tier]}</span>
                        {item.status !== 'fresh' && (
                          <button
