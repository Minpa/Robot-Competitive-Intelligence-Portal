  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-ink-500 text-sm">벤치마크 데이터 로딩 중...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-ink-100 rounded-xl border border-ink-200 p-6">
        <div className="mb-2">
          <h2 className="text-xl font-bold text-ink-900">THE ULTIMATE BENCHMARK</h2>
          <p className="text-base text-ink-500">완벽한 로봇까지의 거리</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {data.competitors.map(comp => {
          const color = COMPANY_COLORS[comp.slug] || 'rgb(var(--color-ink-500))';
        ...
        <div className="lg:col-span-2">
          {selectedCompetitor ? (
            ...
            <div className="bg-white rounded-xl border border-ink-200 p-4 flex items-center justify-center h-full">
              <p className="text-ink-500 text-base">회사를 선택하세요</p>
            </div>
          )}
        </div>
      </div>
