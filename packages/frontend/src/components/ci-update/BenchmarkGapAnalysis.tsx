  ...
  if (!cloid) {
    return (
      <div className="bg-white rounded-xl border border-ink-200 p-4">
        <h3 className="text-base font-semibold text-ink-900">CLOiD 갭 분석</h3>
        <p className="text-ink-500 text-sm mt-2">CLOiD 데이터가 없습니다.</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="bg-green-500/5 border border-green-500/20 rounded-lg p-3">
          ...
          <div className="space-y-2">
            {strengths.length === 0 && <p className="text-sm text-ink-500">해당 없음</p>}
            {strengths.map(({ axis, current, target }) => (
              <div key={axis.key} className="flex items-center justify-between text-sm">
                <span className="text-ink-700">{axis.label}</span>
                <span className="text-green-400">
                  {current}{target > current ? ` → ${target}` : ''} <span className="text-ink-500">/ 10</span>
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-3">
          ...
          <div className="space-y-2">
            {gaps.length === 0 && <p className="text-sm text-ink-500">해당 없음</p>}
            {gaps.filter(g => !strengthKeys.has(g.axis.key) || g.gap >= 5).map(({ axis, current, target, gap }) => (
              <div key={axis.key} className="flex items-center justify-between text-sm">
                <span className="text-ink-700">{axis.label}</span>
                <span className="text-red-400">
                  {current} → {target} <span className="text-ink-500">(갭 {gap})</span>
                </span>
              </div>
            ))}
          </div>
