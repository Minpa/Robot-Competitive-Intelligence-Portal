...
export default function ScoringPipelineAdmin() {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Last run status */}
      {statusLoading ? (
        <div className="text-xs text-slate-400">상태 로딩 중...</div>
      ) : lastStatus ? (
        <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-800 rounded px-3 py-2">
          {statusIcon}
          <span>
        ...
        <div className="text-xs text-slate-400">실행 기록 없음</div>
      )}

      {/* DB Migration section */}
      <div className="border border-slate-700 rounded p-3 space-y-2">
        <div className="text-xs font-medium text-slate-400">DB 마이그레이션</div>
        <button
          ...
          {result.errors && result.errors.length > 0 && (
            <div className="px-3 py-2 space-y-1 bg-slate-900">
              ...
              {result.errors.map((err, i) => (
                <div key={i} className="text-xs text-slate-400 pl-2 border-l-2 border-red-700">
...
function RobotRecalcList({
  ...
  return (
    <div className="space-y-1">
      <div className="text-xs font-medium text-slate-400">로봇별 재계산</div>
