...
export default function ScoringPipelineAdmin() {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Last run status */}
      {statusLoading ? (
        <div className="text-xs text-gray-400">상태 로딩 중...</div>
      ) : lastStatus ? (
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-800 rounded px-3 py-2">
          {statusIcon}
          <span>
      ...
      {fullRunMutation.isError && (
        <div className="text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 rounded px-3 py-2">
        ...
        <div className="rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          {result.errors && result.errors.length > 0 && (
            <div className="px-3 py-2 space-y-1 bg-white dark:bg-slate-900">
              <div className="text-xs font-medium text-red-600 dark:text-red-400">오류 상세:</div>
              {result.errors.map((err, i) => (
                <div key={i} className="text-xs text-gray-600 dark:text-gray-400 pl-2 border-l-2 border-red-300 dark:border-red-700">
...
function RobotRecalcList({
  ...
  return (
    <div className="space-y-1">
      <div className="text-xs font-medium text-gray-500 dark:text-gray-400">로봇별 재계산</div>
      <div className="max-h-40 overflow-y-auto space-y-0.5">
        {robots.map((robot: any) => (
          <div
            key={robot.id}
            className="flex items-center justify-between text-xs py-1 px-2 rounded hover:bg-gray-50 dark:hover:bg-slate-800"
          >
            ...
            <button
              ...
              disabled={isRunning}
              className="flex-shrink-0 flex items-center gap-1 px-2 py-0.5 text-xs text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
