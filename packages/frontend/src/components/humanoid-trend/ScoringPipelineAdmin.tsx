  ...
  return (
    <div className="space-y-3">
      ...
      {statusLoading ? (
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 bg-slate-800 rounded px-3 py-2">
          {statusIcon}
          <span>
      ...
      <div className="border border-gray-700 rounded p-3 space-y-2">
        <div className="text-xs font-medium text-gray-400">DB 마이그레이션</div>
        <button
        ...
        {migrationResult && (
          <div className="text-xs space-y-1">
            ...
            {migrationResult.results?.map((r: any, i: number) => (
              <div key={i} className={`pl-2 border-l-2 ${r.status === 'success' ? 'border-green-300 text-gray-400' : 'border-red-300 text-red-500'}`}>
      ...
      {result && (
        <div className="rounded border border-gray-700 overflow-hidden">
          ...
          {result.errors && result.errors.length > 0 && (
            <div className="px-3 py-2 space-y-1 bg-slate-900">
              <div className="text-xs font-medium text-red-400">오류 상세:</div>
              {result.errors.map((err, i) => (
                <div key={i} className="text-xs text-gray-400 pl-2 border-l-2 border-red-700">
...
function RobotRecalcList({
  ...
  return (
    <div className="space-y-1">
      <div className="text-xs font-medium text-gray-400">로봇별 재계산</div>
      <div className="max-h-40 overflow-y-auto space-y-0.5">
        {robots.map((robot: any) => (
          <div
            key={robot.id}
            className="flex items-center justify-between text-xs py-1 px-2 rounded hover:bg-slate-800"
          >
