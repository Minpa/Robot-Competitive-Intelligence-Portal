                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-lg border p-2 text-left transition-colors ${
                    isSelected
                      ...
                      : 'border-ink-200 bg-ink-100 hover:border-ink-200'
                  }`}
                >
                  <div
                    className={`flex h-4 w-4 flex-shrink-0 items-center justify-center rounded border ${
                      isSelected ? 'border-blue-500 bg-blue-500' : 'border-ink-200'
                    }`}
                  >
                    {isSelected && <Check className="h-3 w-3 text-white" />}
                  </div>
                  <div className="min-w-0">
                    <p className="truncate text-xs font-medium text-ink-900">{r.robotName}</p>
                    <p className="truncate text-[10px] text-ink-500">{r.companyName} · {r.averageScore.toFixed(1)}</p>
                  </div>
                  <span className="ml-auto flex-shrink-0 text-[10px] text-ink-500">#{i + 1}</span>
                </button>
              );
            })}
        </div>
