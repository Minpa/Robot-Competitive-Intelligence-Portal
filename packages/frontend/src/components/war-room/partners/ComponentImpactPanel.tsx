...
export function ComponentImpactPanel({ partners, isLoading }: Props) {
      ...
      <div className="rounded-lg bg-argos-surface border border-argos-borderSoft p-4">
        ...
        <div className="space-y-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-8 bg-argos-bgAlt rounded animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-lg bg-argos-surface border border-argos-borderSoft p-4">
      <h3 className="text-sm font-semibold text-argos-ink mb-4">부품 영향도 분석</h3>
      {rows.length === 0 ? (
        <p className="text-xs text-argos-muted text-center py-4">부품 파트너 데이터 없음</p>
      ) : (
        <div className="space-y-2">
          {rows.map((r) => (
            <div key={r.sub} className="flex items-center gap-3">
              <span className="text-xs text-argos-inkSoft w-20 shrink-0">{r.label}</span>
              <div className="flex-1 h-4 bg-argos-bgAlt rounded-full overflow-hidden">
                <div
                  className="h-full bg-blue-500/70 rounded-full"
                  style={{ width: `${(r.avgCompat / 10) * 100}%` }}
                />
              </div>
              <span className="text-xs text-argos-muted w-16 text-right">
                호환 {r.avgCompat.toFixed(1)}
              </span>
              <span className="text-xs text-argos-muted w-10 text-right">{r.count}개</span>
            </div>
          ))}
        </div>
      )}
