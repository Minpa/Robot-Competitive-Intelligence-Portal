...
export function ComponentImpactPanel({ partners, isLoading }: Props) {
      ...
      <div className="rounded-lg bg-white border border-ink-100 p-4">
        ...
        <div className="space-y-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-8 bg-ink-100 rounded animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-lg bg-white border border-ink-100 p-4">
      <h3 className="text-sm font-semibold text-ink-900 mb-4">부품 영향도 분석</h3>
      {rows.length === 0 ? (
        <p className="text-xs text-ink-500 text-center py-4">부품 파트너 데이터 없음</p>
      ) : (
        <div className="space-y-2">
          {rows.map((r) => (
            <div key={r.sub} className="flex items-center gap-3">
              <span className="text-xs text-ink-700 w-20 shrink-0">{r.label}</span>
              ...
              <span className="text-xs text-ink-500 w-10 text-right">{r.count}개</span>
            </div>
          ))}
        </div>
      )}
