...
export function PartnerGrid({ partners, isLoading, onSelect, selectedId }: Props) {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="h-36 animate-pulse rounded-lg bg-slate-800/50" />
        ))}
      </div>
    );
  }
  ...
  if (partners.length === 0) {
    return (
      <div className="rounded-lg bg-slate-800/40 p-8 text-center text-slate-400">
        해당 조건의 파트너가 없습니다.
      </div>
    );
  }
      ...
      {partners.map((p) => (
        ...
        >
          ...
          <div className="flex items-center gap-1 text-xs text-slate-400 mb-1">
            ...
            {p.subCategory && (
              <span className="text-slate-500">/ {p.subCategory}</span>
            )}
          </div>
