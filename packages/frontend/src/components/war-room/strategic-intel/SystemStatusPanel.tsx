...
function StatusBadge({ status }: { status: string }) {
  ...
  }[status] || { bg: 'bg-ink-100', text: 'text-ink-500', icon: Clock };
...
export function SystemStatusPanel() {
  ...
  if (schedulerLoading) {
    return <div className="animate-pulse bg-white rounded-xl h-48" />;
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-ink-100 p-6">
      {/* Header */}
      <h3 className="text-lg font-semibold text-ink-900 flex items-center gap-2 mb-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {tasks.map((t: any) => (
                <div key={t.name} className="flex items-center justify-between p-3 bg-ink-100 rounded-lg">
                  <div>
                    <div className="text-sm text-ink-900">{t.label}</div>
              ...
              {tasks.length === 0 && (
                <p className="text-xs text-ink-500">스케줄러가 초기화되지 않았습니다.</p>
              )}
            </div>
          </div>
          ...
          <div className="p-4 bg-ink-100 rounded-lg">
            ...
            <div className="flex items-end gap-2 mb-2">
              ...
              <span className="text-sm text-ink-500 mb-0.5">/ ${budget?.limitUsd?.toFixed(2) ?? '7.00'}</span>
            </div>
        ...
        <div>
          <h4 className="text-sm font-medium text-ink-700 mb-3">파이프라인 실행 이력</h4>
          {historyLoading ? (
            <div className="animate-pulse bg-ink-100 rounded-lg h-40" />
          ) : runs.length === 0 ? (
            <p className="text-xs text-ink-500">실행 이력이 없습니다.</p>
          ) : (
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              {runs.map((run: any) => (
                <div key={run.id} className="p-3 bg-ink-100 rounded-lg">
