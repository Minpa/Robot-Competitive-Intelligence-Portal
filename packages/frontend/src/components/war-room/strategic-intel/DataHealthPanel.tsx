...
function CompletenessBar({ value, size = 'sm' }: { value: number; size?: 'sm' | 'lg' }) {
  ...
  return (
    <div className={`w-full ${h} rounded-full bg-slate-700 overflow-hidden`}>
  ...
  if (error || !data || data.message) {
    return (
      <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
        ...
        <p className="text-slate-400 text-sm">아직 감사 리포트가 없습니다. 감사를 실행해주세요.</p>
      </div>
    );
  }
    ...
    <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        <div className="bg-slate-900/50 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-white">{report.totalRobots}</div>
          <div className="text-xs text-slate-400">전체 로봇</div>
        </div>
        <div className="bg-slate-900/50 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-blue-400">{Math.round(report.averageCompleteness)}%</div>
          <div className="text-xs text-slate-400">평균 완성도</div>
        ...
        <div className="bg-amber-500/10 rounded-lg p-3 text-center">
          ...
          <div className="text-xs text-slate-400">주의</div>
        </div>
        <div className="bg-emerald-500/10 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-emerald-400">{report.okCount}</div>
          <div className="text-xs text-slate-400">양호</div>
        </div>
      </div>
      ...
      {report.topMissingSpecs && report.topMissingSpecs.some((s: any) => s.missingCount > 0) && (
        <div className="mb-6 p-3 bg-slate-900/50 rounded-lg">
          <h4 className="text-sm font-medium text-slate-300 mb-2">가장 많이 누락된 스펙</h4>
          <div className="flex flex-wrap gap-2">
            {report.topMissingSpecs.filter((s: any) => s.missingCount > 0).map((s: any) => (
              <span key={s.specType} className="px-2 py-1 bg-slate-800 rounded text-xs text-slate-300">
      ...
      <div className="space-y-2 max-h-[500px] overflow-y-auto">
        <div className="grid grid-cols-[200px_60px_1fr_80px] gap-2 text-xs text-slate-500 font-medium px-2 sticky top-0 bg-slate-800/90 py-2 rounded">
          <span>로봇</span>
          <span>상태</span>
          <span>스펙 완성도</span>
        ...
        {robots.map((r: any) => {
          ...
          const Icon = cfg.icon;
          return (
            <div key={r.robotId} className="grid grid-cols-[200px_60px_1fr_80px] gap-2 items-center px-2 py-2 rounded-lg hover:bg-slate-700/30 transition-colors">
              <div className="truncate">
                <span className="text-sm text-white">{r.robotName}</span>
                {r.companyName && (
                  <span className="text-xs text-slate-500 ml-1">{r.companyName}</span>
                )}
              </div>
