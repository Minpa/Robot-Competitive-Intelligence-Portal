...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg border border-argos-border bg-argos-surface p-2 text-xs shadow-lg">
      ...
      <p className="mt-1 text-argos-inkSoft">PoC: {d.x.toFixed(1)}</p>
      <p className="text-argos-inkSoft">RFM: {d.y.toFixed(1)}</p>
      <p className="text-argos-inkSoft">종합: {(d.x + d.y).toFixed(1)}</p>
    </div>
  );
}
...
function BubbleChartInfo() {
  return (
    <ScoreInfoButton title="포지셔닝 시프트 차트 해석">
      <p className="text-argos-muted">What-If 시뮬레이션 결과를 경쟁사 대비 포지션으로 시각화합니다.</p>

      <div className="rounded-lg bg-argos-bgAlt p-3">
        <p className="font-medium text-argos-ink mb-1">축 설명</p>
      ...
      <div className="rounded-lg bg-argos-bgAlt p-3">
        <p className="font-medium text-argos-ink mb-2">사분면 해석</p>
        <div className="space-y-1.5 text-xs">
          ...
          <p><span className="text-argos-muted">좌하단 (초기 단계)</span> — 양쪽 모두 개선 필요</p>
        </div>
      </div>

      <div className="rounded-lg bg-argos-bgAlt p-3">
        <p className="font-medium text-argos-ink mb-1">점 설명</p>
        <div className="mt-1 space-y-1 text-xs">
          <p><span className="text-argos-muted">● Before (회색)</span> — 현재 LG 로봇의 위치</p>
      ...
      <div className="rounded-lg bg-argos-bgAlt p-3">
        <p className="font-medium text-argos-ink mb-1">비즈니스 활용</p>
        <p className="text-xs text-argos-muted">스펙을 변경하며 LG 로봇이 경쟁사 대비 어느 위치로 이동하는지 시뮬레이션합니다. 목표는 우상단(리더 영역)으로 이동하는 것이며, 어떤 투자가 가장 효과적인지 판단하는 데 활용합니다.</p>
      </div>
    </ScoreInfoButton>
  );
}
  ...
  return (
    <div className="rounded-lg bg-argos-surface border border-argos-borderSoft p-4">
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-sm font-semibold text-argos-ink">포지셔닝 시프트</h3>
        <BubbleChartInfo />
      </div>
