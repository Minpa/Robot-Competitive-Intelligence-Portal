...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-800 p-2 text-xs shadow-lg">
      ...
      <p className="mt-1 text-slate-300">PoC: {d.x.toFixed(1)}</p>
      <p className="text-slate-300">RFM: {d.y.toFixed(1)}</p>
      <p className="text-slate-300">종합: {(d.x + d.y).toFixed(1)}</p>
    </div>
  );
}
...
function BubbleChartInfo() {
  return (
    <ScoreInfoButton title="포지셔닝 시프트 차트 해석">
      <p className="text-slate-400">What-If 시뮬레이션 결과를 경쟁사 대비 포지션으로 시각화합니다.</p>

      <div className="rounded-lg bg-slate-800/50 p-3">
        <p className="font-medium text-white mb-1">축 설명</p>
      ...
      <div className="rounded-lg bg-slate-800/50 p-3">
        <p className="font-medium text-white mb-2">사분면 해석</p>
        <div className="space-y-1.5 text-xs">
          ...
          <p><span className="text-slate-400">좌하단 (초기 단계)</span> — 양쪽 모두 개선 필요</p>
        </div>
      </div>

      <div className="rounded-lg bg-slate-800/50 p-3">
        <p className="font-medium text-white mb-1">점 설명</p>
        <div className="mt-1 space-y-1 text-xs">
          <p><span className="text-slate-400">● Before (회색)</span> — 현재 LG 로봇의 위치</p>
      ...
      <div className="rounded-lg bg-slate-800/50 p-3">
        <p className="font-medium text-white mb-1">비즈니스 활용</p>
        <p className="text-xs text-slate-400">스펙을 변경하며 LG 로봇이 경쟁사 대비 어느 위치로 이동하는지 시뮬레이션합니다. 목표는 우상단(리더 영역)으로 이동하는 것이며, 어떤 투자가 가장 효과적인지 판단하는 데 활용합니다.</p>
      </div>
    </ScoreInfoButton>
  );
}
  ...
  return (
    <div className="rounded-lg bg-slate-800/40 border border-slate-700/50 p-4">
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-sm font-semibold text-white">포지셔닝 시프트</h3>
        <BubbleChartInfo />
      </div>
      <ResponsiveContainer width="100%" height={300}>
        <ScatterChart margin={{ top: 15, right: 15, bottom: 25, left: 15 }}>
          ...
          <XAxis
            ...
            label={{ value: 'PoC Total (하드웨어)', position: 'bottom', fill: '#94a3b8', fontSize: 10, offset: 10 }}
          />
          <YAxis
            type="number"
            dataKey="y"
            ...
            domain={[0, maxY]}
            tick={{ fill: '#94a3b8', fontSize: 11 }}
            label={{ value: 'RFM Total (SW/시장)', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 10 }}
          />
          <ZAxis type="number" dataKey="z" range={[80, 350]} />

          {/* Quadrant reference lines */}
          <ReferenceLine x={midX} stroke="#475569" strokeDasharray="6 4">
            <Label value="SW특화 ←" position="insideTopLeft" fill="#64748b" fontSize={9} />
            <Label value="→ 리더" position="insideTopRight" fill="#64748b" fontSize={9} />
          </ReferenceLine>
          <ReferenceLine y={midY} stroke="#475569" strokeDasharray="6 4">
            <Label value="초기단계" position="insideBottomLeft" fill="#64748b" fontSize={9} />
            <Label value="HW특화" position="insideBottomRight" fill="#64748b" fontSize={9} />
          </ReferenceLine>
          ...
          {beforeData.length > 0 && (
            <Scatter name={`${robotName} (현재)`} data={beforeData}>
              <Cell fill="#64748b" fillOpacity={0.7} />
            </Scatter>
          )}
