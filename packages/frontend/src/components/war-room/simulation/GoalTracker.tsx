...
export function GoalTracker() {
  ...
  return (
    <div className="rounded-lg bg-slate-800/40 border border-slate-700/50 p-4">
      ...
      {showForm && (
        <div className="mb-3 rounded-md bg-slate-900/60 border border-slate-700/50 p-3 space-y-2">
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="목표 제목"
            className="w-full rounded-md bg-slate-800 border border-slate-700 px-2 py-1 text-xs text-white placeholder-slate-500"
          />
          <div className="flex gap-2">
            <select
              value={metricType}
              onChange={(e) => setMetricType(e.target.value)}
              className="flex-1 rounded-md bg-slate-800 border border-slate-700 px-2 py-1 text-xs text-white"
            >
            ...
            <input
              ...
              className="w-20 rounded-md bg-slate-800 border border-slate-700 px-2 py-1 text-xs text-white"
            />
          </div>
          <button
      ...
      <div className="space-y-2">
        {(goals ?? []).length === 0 ? (
          <p className="text-xs text-slate-500 text-center py-4">전략 목표 없음</p>
        ) : (
          (goals ?? []).map((g: StrategicGoal) => {
            ...
            return (
              <div key={g.id} className="rounded-md bg-slate-800/60 border border-slate-700/30 p-3">
