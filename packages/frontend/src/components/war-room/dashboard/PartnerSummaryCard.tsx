      ...
      <div className="rounded-xl border border-ink-200 bg-white p-4">
        ...
        <div className="space-y-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-6 w-full animate-pulse rounded bg-ink-100" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="rounded-xl border border-ink-200 bg-white p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-ink-900">전략 파트너 요약</h3>
          <PartnerSummaryInfo />
        </div>
      ...
      {data.length === 0 ? (
        <p className="mt-4 text-sm text-ink-500">파트너 데이터가 없습니다</p>
      ) : (
