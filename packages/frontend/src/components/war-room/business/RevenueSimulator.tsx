...
export function RevenueSimulator() {
  ...
  return (
    <div className="rounded-lg bg-slate-800/40 border border-slate-700/50 p-4">
      <h3 className="text-sm font-semibold text-white mb-4">수익 모델 시뮬레이터</h3>
        ...
        {model.fields.map((f) => (
          <div key={f.key}>
            ...
            <input
              ...
              className="w-full rounded-md bg-slate-900 border border-slate-700 px-3 py-1.5 text-sm text-white focus:border-blue-500 focus:outline-none"
            />
          </div>
        ))}
      </div>
      ...
      {/* Result */}
      <div className="rounded-md bg-slate-900/60 border border-slate-700/50 p-3 text-center">
        <p className="text-xs text-slate-400 mb-1">예상 매출</p>
