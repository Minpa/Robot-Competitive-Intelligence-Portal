...
export function OpportunityMatrix({ domains, isLoading }: Props) {
  if (isLoading) {
    return (
      <div className="rounded-lg bg-white border border-ink-100 p-4">
        ...
        <div className="h-72 bg-ink-100 rounded animate-pulse" />
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg bg-white border border-ink-100 p-4">
      <h3 className="text-sm font-semibold text-ink-900 mb-4">사업화 기회 매트릭스</h3>
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={300}>
          <ScatterChart margin={{ top: 10, right: 20, bottom: 25, left: 10 }}>
            ...
            >
              <Label value="LG 준비도" position="bottom" offset={5} fill="rgb(var(--color-slate-400))" fontSize={11} />
            </XAxis>
            <YAxis
              type="number"
              dataKey="y"
              tick={{ fill: 'rgb(var(--color-slate-400))', fontSize: 11 }}
            >
              <Label value="SOM ($B)" angle={-90} position="insideLeft" fill="rgb(var(--color-slate-400))" fontSize={11} />
            </YAxis>
