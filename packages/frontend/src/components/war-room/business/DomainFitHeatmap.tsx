...
function fitColor(score: number | null): string {
  if (score == null) return 'bg-ink-100 text-ink-400';
...
export function DomainFitHeatmap({ data, isLoading }: Props) {
  ...
  if (domainNames.length === 0) {
    return (
      <div className="rounded-lg bg-white border border-ink-100 p-4">
        <h3 className="text-sm font-semibold text-ink-900 mb-4">로봇-분야 적합도</h3>
    ...
    <div className="rounded-lg bg-white border border-ink-100 p-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr>
              <th className="text-left text-ink-500 pb-2 pr-2 font-medium">분야</th>
              {robotNames.map((r) => (
                <th key={r} className="text-center text-ink-500 pb-2 px-1 font-medium whitespace-nowrap max-w-[80px] truncate">
                  {r}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {domainNames.map((dn) => (
              <tr key={dn}>
                <td className="text-ink-700 py-1 pr-2 whitespace-nowrap">{dn}</td>
                {robotNames.map((rn) => {
