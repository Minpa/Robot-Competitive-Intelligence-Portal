      ...
      <div className="rounded-xl border border-ink-200 bg-paper p-4">
        ...
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-14 w-full animate-pulse rounded bg-ink-100" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-ink-200 bg-paper p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-ink-900">경쟁 알림</h3>
        <span className="ml-auto text-xs text-ink-500">{filtered.length}건</span>
      </div>
      ...
      {/* Alert list */}
      {filtered.length === 0 ? (
        <p className="py-6 text-center text-sm text-ink-500">알림이 없습니다</p>
      ) : (
        <ul className="max-h-[500px] space-y-2 overflow-y-auto">
          {filtered.map((alert) => {
            ...
            return (
              <li
                key={alert.id}
                className={`group cursor-pointer rounded-lg border p-2.5 transition-colors hover:bg-ink-100 ${
                  alert.isRead
              ...
              >
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`truncate text-sm font-medium ${alert.isRead ? 'text-ink-500' : 'text-ink-900'}`}>
                      {alert.title}
                    </p>
                    {alert.summary && (
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    <span className="text-xs text-ink-500">{formatRelativeTime(alert.createdAt)}</span>
                    {!alert.isRead && (
                      <button
                        type="button"
                        onClick={(e) => {
