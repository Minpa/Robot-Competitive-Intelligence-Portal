...
export function LgRobotList({ robots, selectedId, onSelect, onCreateClick, isLoading }: LgRobotListProps) {
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white">LG 로봇 목록</h3>
        <button
      ...
      {robots.length === 0 ? (
        <p className="text-sm text-slate-500">등록된 LG 로봇이 없습니다</p>
      ) : (
            ...
            <li key={robot.id}>
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-white">{robot.name}</p>
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    {robot.locomotionType && (
                      <span className="text-xs text-slate-400">{robot.locomotionType}</span>
                    )}
                  </div>
