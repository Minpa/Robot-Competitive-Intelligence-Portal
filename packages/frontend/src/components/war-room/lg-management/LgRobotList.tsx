...
export function LgRobotList({ robots, selectedId, onSelect, onCreateClick, isLoading }: LgRobotListProps) {
    ...
    return (
      <div className="rounded-xl border border-ink-200 bg-white p-4">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="h-8 w-8 animate-pulse rounded bg-ink-100" />
        </div>
        <div className="space-y-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-16 w-full animate-pulse rounded-lg bg-ink-100" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-ink-200 bg-white p-4">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink-900">LG 로봇 목록</h3>
        <button
      ...
      {robots.length === 0 ? (
        <p className="text-sm text-ink-500">등록된 LG 로봇이 없습니다</p>
      ) : (
              ...
              <button
                ...
                className={`flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors ${
                  selectedId === robot.id
                    ...
                    : 'border-ink-200 bg-ink-100 hover:bg-ink-100'
                }`}
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-ink-900">{robot.name}</p>
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    {robot.locomotionType && (
                      <span className="text-xs text-ink-500">{robot.locomotionType}</span>
                    )}
                  </div>
