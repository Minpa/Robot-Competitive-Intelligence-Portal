      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        ...
        <div className="space-y-2">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-14 w-full animate-pulse rounded-lg bg-slate-800" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-white">변경 이력</h3>
      </div>
        ...
        <ul className="max-h-64 space-y-2 overflow-y-auto">
          {sorted.map((log) => (
            ...
            >
              ...
              {log.changedBy && (
                <p className="mt-1 text-xs text-slate-500">편집자: {log.changedBy}</p>
              )}
            </li>
          ))}
        </ul>
