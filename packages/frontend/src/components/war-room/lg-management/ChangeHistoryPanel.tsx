      ...
      <div className="rounded-xl border border-ink-200 bg-white p-4">
        ...
        <div className="space-y-2">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-14 w-full animate-pulse rounded-lg bg-ink-100" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-ink-200 bg-white p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-ink-900">변경 이력</h3>
      </div>

      {sorted.length === 0 ? (
        <p className="text-sm text-ink-500">변경 이력이 없습니다</p>
      ) : (
        <ul className="max-h-64 space-y-2 overflow-y-auto">
          {sorted.map((log) => (
            <li
              key={log.id}
              className="rounded-lg border border-ink-200 bg-ink-100 p-2.5"
            >
              ...
              {log.changedBy && (
                <p className="mt-1 text-xs text-ink-500">편집자: {log.changedBy}</p>
              )}
            </li>
          ))}
        </ul>
