...
export function EntityLinkingPanel({ candidates, unmatched, onConfirm, isLoading }: EntityLinkingPanelProps) {
  ...
  return (
    <div className="bg-argos-surface border border-argos-border rounded-xl p-6 space-y-4">
      ...
      <p className="text-sm text-argos-muted">추출된 엔티티를 기존 DB와 연결하거나 신규 생성하세요.</p>

      <div className="space-y-3 max-h-96 overflow-y-auto">
        {Object.entries(candidates).map(([name, cands]) => (
          <div key={name} className="bg-argos-bgAlt rounded-lg p-3">
            <p className="text-sm font-medium text-argos-ink mb-2">{name}</p>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded text-sm transition-all ${
                    selectedLinks[name] === c.entityId
                      ...
                      : 'bg-argos-bgAlt text-argos-inkSoft hover:bg-argos-bgAlt'
                  }`}
                >
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded text-sm transition-all ${
                  createNew.has(name)
                    ...
                    : 'bg-argos-bgAlt text-argos-muted hover:bg-argos-bgAlt'
                }`}
              >
        ...
        {unmatched.filter(u => !Object.keys(candidates).includes(u.name)).map(u => (
          <div key={u.name} className="bg-argos-bgAlt rounded-lg p-3">
            <p className="text-sm font-medium text-argos-ink mb-2">{u.name} <span className="text-xs text-argos-muted">({u.type})</span></p>
            <p className="text-xs text-argos-muted mb-2">매칭 후보 없음</p>
            <button
