...
export function EntityLinkingPanel({ candidates, unmatched, onConfirm, isLoading }: EntityLinkingPanelProps) {
  ...
  return (
    <div className="bg-white border border-ink-200 rounded-xl p-6 space-y-4">
      ...
      <p className="text-sm text-ink-500">추출된 엔티티를 기존 DB와 연결하거나 신규 생성하세요.</p>

      <div className="space-y-3 max-h-96 overflow-y-auto">
        {Object.entries(candidates).map(([name, cands]) => (
          <div key={name} className="bg-ink-100 rounded-lg p-3">
            <p className="text-sm font-medium text-ink-900 mb-2">{name}</p>
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded text-sm transition-all ${
                  createNew.has(name)
                    ...
                    : 'bg-ink-100 text-ink-500 hover:bg-ink-100'
                }`}
              >
        ...
        {unmatched.filter(u => !Object.keys(candidates).includes(u.name)).map(u => (
          <div key={u.name} className="bg-ink-100 rounded-lg p-3">
            <p className="text-sm font-medium text-ink-900 mb-2">{u.name} <span className="text-xs text-ink-500">({u.type})</span></p>
            <p className="text-xs text-ink-500 mb-2">매칭 후보 없음</p>
            <button
