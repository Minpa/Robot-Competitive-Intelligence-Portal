  ...
  return (
    <div className="px-4 py-6 space-y-6 max-w-full overflow-hidden">
      ...
      <div className="bg-argos-surface rounded-xl border border-argos-border p-4">
        <h2 className="text-sm font-semibold text-argos-inkSoft mb-3">주요 검색 사이트</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {EXTERNAL_LINKS.map(link => (
            <a
              ...
              className="flex items-start gap-2 p-3 rounded-lg bg-argos-surface border border-argos-border hover:border-argos-blue/30 hover:bg-argos-bgAlt transition-colors group"
            >
              <ExternalLink className="w-4 h-4 text-argos-faint group-hover:text-blue-400 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-medium text-argos-inkSoft group-hover:text-blue-400">{link.name}</div>
                <div className="text-xs text-argos-faint mt-0.5">{link.desc}</div>
              </div>
            </a>
          ))}
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[250px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-argos-surface border border-argos-border rounded-lg text-sm text-argos-ink placeholder-argos-faint focus:outline-none focus:ring-2 focus:ring-argos-blue focus:border-transparent"
          />
        </div>
        <select
          value={categoryFilter}
          onChange={e => setCategoryFilter(e.target.value)}
          className="bg-argos-surface border border-argos-border text-argos-inkSoft text-sm rounded-lg px-3 py-2 focus:ring-argos-blue focus:border-argos-blue"
        >
      ...
      {/* Results */}
      <div className="text-xs text-argos-faint mb-2">{filtered.length}건 검색됨</div>
      <div className="space-y-3">
        {filtered.map(project => (
          <div
            key={project.id}
            className="bg-argos-surface rounded-xl border border-argos-border p-4 hover:border-argos-blue/30 transition-colors"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-argos-faint">{project.category}</span>
                </div>
                <h3 className="text-sm font-semibold text-argos-ink mb-2">{project.title}</h3>
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-argos-muted">
                ...
                <div className="flex gap-1.5 mt-2">
                  {project.keywords.map(kw => (
                    <span
                      key={kw}
                      className="flex items-center gap-0.5 text-[11px] px-2 py-0.5 rounded-full bg-argos-chip/50 text-argos-muted"
                    >
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 text-argos-faint">
          검색 결과가 없습니다
        </div>
      )}
    </div>
