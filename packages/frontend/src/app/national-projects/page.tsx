  ...
  return (
    <div className="min-h-screen bg-slate-950">
      <div className="px-4 py-6 space-y-6 max-w-full overflow-hidden">
        <div>
          <h1 className="text-2xl font-bold text-slate-200">국내 국책과제 검색</h1>
          <p className="text-sm text-slate-400 mt-1">
            로봇 관련 국가 R&D 과제 현황 및 주요 기관 링크
          </p>
        </div>

        {/* External Links */}
        <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-4">
          <h2 className="text-sm font-semibold text-slate-300 mb-3">주요 검색 사이트</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {EXTERNAL_LINKS.map(link => (
              <a
                ...
                className="flex items-start gap-2 p-3 rounded-lg bg-slate-900/50 border border-slate-700 hover:border-blue-500/50 hover:bg-slate-800/50 transition-colors group"
              >
                <ExternalLink className="w-4 h-4 text-slate-500 group-hover:text-blue-400 mt-0.5 shrink-0" />
                <div>
                  <div className="text-sm font-medium text-slate-300 group-hover:text-blue-400">{link.name}</div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <select
            ...
            className="bg-slate-800 border border-slate-700 text-slate-300 text-sm rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
          >
        ...
        {/* Results */}
        <div className="text-xs text-slate-500 mb-2">{filtered.length}건 검색됨</div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-1.5 mt-2">
                    {project.keywords.map(kw => (
                      <span
                        key={kw}
                        className="flex items-center gap-0.5 text-[11px] px-2 py-0.5 rounded-full bg-slate-700/50 text-slate-400"
                      >
