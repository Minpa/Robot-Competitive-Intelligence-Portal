...
import type { CiMatrixData, CiCompetitor, CiNewCompetitorRequest, CiValueUpdateRequest, CiValueHistoryEntry, CiFreshnessSummary, CiStagingEntry, CiMonitorAlert, BenchmarkData, HandBenchmarkData } from '@/types/ci-update';
...
class ApiClient {
  ...
  async updateVisionRobotCost(id: string, data: any) {
    return this.request<any>(`/vision-cost/robot-costs/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  }
  ...
  async deleteVisionRobotCost(id: string) {
    return this.request<void>(`/vision-cost/robot-costs/${id}`, {
      method: 'DELETE',
    });
  }
  ...
  async getHandBenchmarkData(): Promise<HandBenchmarkData> {
    return this.request('/ci-update/hand-benchmark');
  }

  async updateHandBenchmarkScore(competitorId: string, axisKey: string, currentScore: number, targetScore: number): Promise<{ success: boolean }> {
    return this.request(`/ci-update/hand-benchmark/scores/${competitorId}`, {
      method: 'PUT',
      body: JSON.stringify({ axisKey, currentScore, targetScore }),
    });
  }
